...
build:
	...
	go build -o metad -ldflags="\
    	-X 'github.com/metaplayer-one/metad/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/metaplayer-one/metad/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/metaplayer-one/metad/versioning.Branch=$(BRANCH)'\
		-X 'github.com/metaplayer-one/metad/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/metad .
	env EDGE_BINARY=${PWD}/artifacts/metad go test -v -timeout=30m ./e2e/...
