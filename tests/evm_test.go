...
import (
	...
	"github.com/metaplayer-one/metad/helper/hex"
	"github.com/metaplayer-one/metad/helper/keccak"
	"github.com/metaplayer-one/metad/state"
	"github.com/metaplayer-one/metad/state/runtime"
	"github.com/metaplayer-one/metad/state/runtime/evm"
	"github.com/metaplayer-one/metad/types"
	"github.com/hashicorp/go-hclog"
	"github.com/umbracle/fastrlp"
)

var chainChainConfig = chain.Params{
	Forks: &chain.Forks{
...
func testVMCase(t *testing.T, name string, c *VMCase) {
	...
	config := chainChainConfig.Forks.At(uint64(env.Number))

	executor := state.NewExecutor(&chainChainConfig, s, hclog.NewNullLogger())
